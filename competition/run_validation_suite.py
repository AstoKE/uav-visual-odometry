...
def run_scenario(label: str, health_scenario: str, n_frames: int,
    ...
    for i in range(n_frames):
        ...
        wx, wy, _ = est.update(float(deltas[i, 0]), float(deltas[i, 1]),
    ...
    # ── Metrik hesapla ────────────────────────────────────────────────────────
    errs = np.sqrt((est_x_arr - gt_pos[:, 0])**2 + (est_y_arr - gt_pos[:, 1])**2)
    dead_mask = health == 0
    n_dead    = int(dead_mask.sum())

    rmse_2d    = float(np.sqrt(np.mean(errs[dead_mask]**2))) if n_dead > 0 else float("nan")
    final_drift = float(errs[-1])
    max_drift  = float(np.max(errs[dead_mask])) if n_dead > 0 else float("nan")
    ...
    return {
        ...
        "rmse_2d":        round(rmse_2d,    4) if not math.isnan(rmse_2d)    else "N/A",
        "final_drift":    round(final_drift, 4),
        "max_drift":      round(max_drift,  4) if not math.isnan(max_drift)  else "N/A",
...
_HDR = (
    ...
    f"{'Dead%':>6} {'RMSE_2D':>8} {'FinalDrift':>10} {'MaxDrift':>9} "
...
def _fmt_row(r: dict) -> str:
    return (
        ...
        f"{r['n_dead_pct']:>5.1f}% {str(r['rmse_2d']):>8} {str(r['final_drift']):>10} "
