#!/usr/bin/env python3
"""
evaluate_figure8_v3_stride1.py — Figure-8 v3 stride=1 SLAM sonucunu değerlendirir.

stride=1 vs stride=3 farkı:
  - DROID-SLAM tüm 900 frame'i işler (stride=3'te 300 frame)
  - ~900 SLAM çıktı pozu → daha yüksek temporal çözünürlük
  - GT timing: rec_frame = slam_frame * 1

Karşılaştırma tablosu:
  Lawnmower baseline | Figure-8 v2 | Figure-8 v3 (s=3) | Figure-8 v3 (s=1)

Kullanım:
    python3 ~/code/uav-visual-odometry/slam/scripts/evaluate_figure8_v3_stride1.py
"""
...
STRIDE = 1   # stride=1: her frame kullanıldı

TRAJ_IN   = os.path.join(REPO, "slam/outputs/trajectory_figure8_v3_stride1.csv")
DELTA_OUT = os.path.join(REPO, "slam/outputs/delta_trajectory_figure8_v3_stride1.csv")
GT_OUT    = os.path.join(REPO, "evaluation/ground_truth_figure8_v3_stride1.csv")
WORLD_OUT = os.path.join(REPO, "slam/outputs/delta_figure8_v3_stride1_world.csv")
PLOTS_DIR = os.path.join(REPO, "evaluation/plots")
METRICS   = os.path.join(REPO, "evaluation/metrics/rmse_figure8_v3_stride1.txt")
...
PREV = {
    ...
    "Figure-8 v3 stride=3":         3.852,   # önceki v3 sonucu
}
...
def main():
    ...
    if not os.path.isfile(TRAJ_IN):
        ...
        print("  Önce çalıştır: bash slam/scripts/run_droid_figure8_v3_stride1.sh")
        sys.exit(1)

    traj = load_csv(TRAJ_IN, "x", "y", "z")
    n_slam = len(traj)
    print(f"[eval_v3_s1] SLAM frame sayısı : {n_slam}")
    ...
    print(f"[eval_v3_s1] Kullanılan frame  : {n}")

    # ── 4. Scale (path-length) ────────────────────────────────────────────────
    slam_path  = path_length_2d(delta[:, :2])
    gt_path    = path_length_2d(gt)
    path_scale = gt_path / slam_path if slam_path > 1e-9 else 1.0
    print(f"[eval_v3_s1] SLAM 2D yol       : {slam_path:.6f}")
    print(f"[eval_v3_s1] GT   2D yol       : {gt_path:.4f} m")
    print(f"[eval_v3_s1] Path-length scale : {path_scale:.4f}×")
    ...
    slam_world = np.array([best_fn(dx*path_scale, dy*path_scale, dz*path_scale)
                           for dx, dy, dz in delta])
    print(f"[eval_v3_s1] En iyi dönüşüm   : {best_name}  RMSE_2D={best_r2d:.4f}m")
    ...
    print(f"[eval_v3_s1] scale_x           : {scale_x:.4f}  (n={len(sx_vals)}, std={np.std(sx_vals):.2f})")
    print(f"[eval_v3_s1] scale_y           : {scale_y:.4f}  (n={len(sy_vals)}, std={np.std(sy_vals):.2f})")
    ...
    all_rows = list(PREV.items()) + [
        ...
        ("Figure-8 v3 stride=1 ◄",   r2d_ax),
    ]

    print()
    print("=" * 65)
    print("KARŞILAŞTIRMA TABLOSU (stride=1 vs önceki)")
    print("=" * 65)
    ...
    print("=" * 65)
    print(f"\nstride=1  vs  stride=3  : {imp_vs_v3:+.1f}%")
    print(f"stride=1  vs  v2 axis   : {imp_vs_v2:+.1f}%")
    print(f"stride=1  vs  baseline  : {imp_vs_base:+.1f}%")
    ...
    for ax, slam_xy, title_sfx in [
        ...
        ax.set_title(f"Figure-8 v3 stride=1 — {title_sfx}\n(4m kamera, zengin sahne)")
        ax.legend(fontsize=8); ax.grid(True, alpha=0.3); ax.set_aspect("equal")
    fig.tight_layout()
    p = os.path.join(PLOTS_DIR, "trajectory_figure8_v3_stride1.png")
    fig.savefig(p, dpi=120); plt.close(fig)
    print(f"[eval_v3_s1] Trajectory plot : {p}")

    # --- 4-way RMSE karşılaştırma bar chart ---
    compare_rows = [
        ("Lawnmower\nbaseline",         12.983),
        ("Figure-8\nv2 axis",            5.520),
        ("Figure-8 v3\nstride=3",        v3_s3),
    ...
    ax.set_title("RMSE_2D Karşılaştırması — stride=1 vs stride=3 vs önceki", fontsize=11)
    ax.grid(True, alpha=0.3, axis="y")
    fig.tight_layout()
    p2 = os.path.join(PLOTS_DIR, "rmse_comparison_final.png")
    fig.savefig(p2, dpi=120); plt.close(fig)
    print(f"[eval_v3_s1] Karşılaştırma  : {p2}")

    print()
    print("=" * 55)
    print("ÖZET (v3 stride=1)")
    print("=" * 55)
    print(f"  SLAM frame            : {n}")
    print(f"  Stride                : 1  (eskisi: 3)")
    print(f"  Path-length scale     : {path_scale:.1f}×")
    print(f"  scale_x / scale_y     : {scale_x:.2f}×  /  {scale_y:.2f}×")
    print(f"  RMSE_x                : {rx_ax:.4f} m")
    print(f"  RMSE_y                : {ry_ax:.4f} m")
    print(f"  RMSE_2D               : {r2d_ax:.4f} m")
    print(f"  İyileşme vs stride=3  : {imp_vs_v3:+.1f}%")
